...
export const CartProvider = ({ children }) => {
  ...
  const fetchCartItems = async () => {
    try {
      ...
      const response = await axios.get(`${API_BASE}/api/cart`, {  //.env path
  ...
  const addToCart = async (camera) => {
    try {
      ...
      await axios.post(
       `${API_BASE}/api/cart/add`,   // .env path
        {
          cameraId: camera.id,
          quantity: 1,
          name: camera.name,
  ...
  const removeFromCart = async (cameraId) => {
    try {
      const idToken = await auth.currentUser.getIdToken();
      await axios.delete(`${API_BASE}/api/cart/remove/${cameraId}`, {
        headers: { Authorization: `Bearer ${idToken}` },
      });
  ...
  const updateQuantity = async (cameraId, quantity) => {
    ...
    try {
      const idToken = await auth.currentUser.getIdToken();
      await axios.put(
        `${API_BASE}/api/cart/updateQuantity`,   //.env path
        {
          cameraId: cameraId,
          quantity: quantity,
        },
